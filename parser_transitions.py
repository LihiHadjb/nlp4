...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "LA" and len(self.stack) >= 2:
            head = self.stack[-1]
            dependant = self.stack[-2]
            self.dependencies.append((head, dependant))
            self.stack = self.stack[:-2]
            self.stack.append(head)

        if transition == "RA" and len(self.stack) >= 2:
            head = self.stack[-2]
            dependant = self.stack[-1]
            self.dependencies.append((head, dependant))
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        preds = model.predict(minibatch)
        for i, sent in enumerate(minibatch):
            sent.parse([preds[i]])
            if len(sent.buffer) == 0 and len(sent.stack) == 1:
