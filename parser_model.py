...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, t):
        ...
        embbeds = self.pretrained_embeddings(t)
        x = embbeds.view(t.size()[0], self.n_features * self.embed_size)
        ### END YOUR CODE
        return x
    ...
    def forward(self, t):
        ...
        res = self.embedding_lookup(t)
        res = self.embed_to_hidden(res)
        res = F.relu(res)
        res = self.dropout(res)
        logits = self.hidden_to_logits(res)
        ### END YOUR CODE
        return logits
